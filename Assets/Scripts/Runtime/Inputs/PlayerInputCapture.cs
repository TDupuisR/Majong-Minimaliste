...
namespace InputCapture
{
    public class PlayerInputCapture : MonoBehaviour
    {
        ...
        public event Action OnP1MoveStart;
        public event Action OnP1MoveEnd;

        public event Action OnP2MoveStart;
        public event Action OnP2MoveEnd;

        private void OnEnable() {
            _inputAction = new PlayerInput();
            _inputAction.Gameplay.Enable();
            
            _inputAction.Gameplay.MovementPlayer1.performed += ctx => OnP1MoveStart.Invoke();
            _inputAction.Gameplay.MovementPlayer1.canceled += ctx => OnP1MoveEnd.Invoke();
